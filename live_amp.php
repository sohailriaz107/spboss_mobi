<div class="matka-result live-update-card">
	...
	<div class="matka-card live-box">
		...
		$app_link_text = 'Matka Play App <br>(Advance & Fast)<br> Fast Withdrawal';
		...
		<?php if (0) { ?>
			...
			foreach ($theData as $line) {
				...
				if ($gameName == 'TIME BAZAR' || $gameName == 'KALYAN' || $gameName == 'MILAN DAY' || $gameName == 'MILAN NIGHT' || $gameName == 'KALYAN MORNING' || $gameName == 'MILAN MORNING' || $gameName == 'KALYAN NIGHT' || $gameName == 'RAJDHANI DAY' || $gameName == 'RAJDHANI NIGHT' || $gameName == 'MAIN BAZAR' || $gameName == 'SRIDEVI' || $gameName == 'SRIDEVI NIGHT' || $gameName == 'OLD MAIN MUMBAI' || $gameName == 'SUPER DAY' || $gameName == 'KARNATAKA DAY' || $gameName == 'MUMBAI MORNING' || $gameName == 'MAIN KALYAN' || $gameName == 'MUMBAI PAREL') {
					$show_extra_text = 1;
				} else {
					$show_extra_text = 0;
				}
					...
					if ($frequency == 7 || $frequency == 2) {
						if (strpos(date('h:i A', strtotime($timeOpen)), 'PM') && strpos(date('h:i A', strtotime($timeClose)), 'AM')) {
							...
							if ($time1 >= $startTime && $time1 <= $endTime) {
								...
								?>
								<a href="./" class="matka-live-refresh-btn">Refresh</a>
								<?php
						...
						if ($day == "mon") {
							if ($time1 >= $startTime && $time1 <= $endTime) {
								...
								<?php if ($gameName == 'KALYAN' && $app_link_status) { ?>
									...
									<hr>
									<a href="<?php echo $app_link_url; ?>" target="_blank"
										class="matka-live-refresh-btn-2"><?php echo $app_link_text; ?></a>
									<hr>
							...
							<?php } elseif ($time1 >= $starttimeC && $time1 <= $endtimeC) {
								if ($updateDate == date('d-m-Y')) { ?>
									...
									<?php if ($gameName == 'KALYAN' && $app_link_status) { ?>
										...
										<hr><a href="<?php echo $app_link_url; ?>" target="_blank"
											class="matka-live-refresh-btn-2"><?php echo $app_link_text; ?></a>
										<hr>
